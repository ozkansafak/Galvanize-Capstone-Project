...
def generate_a_fugue(epoch, N=96/4*16*2):
	...
	snippet = ''.join(phrase) 
	print("----\n %s \n----" % snippet)
	pickle.dump(fugue, open('fugue_N'+str(N)+'epoch'+str(int(epoch))+'.p', 'wb'))
	return
...
'''/\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\ 
   ||  ||  ||  ||  ||  ||  ||  ||  ||  ||  ||  ||  ||  ||  ||  ||  ||  ||  ||  ||  ||  ||  
   \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  '''
def make_batch(p=0, pitch_matrix=pitch_matrix, batch_size=BATCH_SIZE):
...
pitch_matrix = pitch_matrix[0:1600]
...
for it in xrange(data_size * NUM_EPOCHS / BATCH_SIZE):
	epoch = it*1.0*PRINT_FREQ/data_size*BATCH_SIZE
	print "it: {}".format(it)
	t4 = time.time()
